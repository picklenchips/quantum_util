...
def plot_wf_cube(
    ax: Axes, corner1, corner2, color="orange", linestyle="dashed", linewidth=1
):
    x1, y1, z1 = corner1
    x2, y2, z2 = corner2
    ax.plot(
...
def label_str_states(N):
    """return ordered string of |00>, |01>, |10>, |11> over N qubits"""
    ret = []
    for i in range(2**N):
        string = "$|"
...
def plot_op(
    operators: list, titles=[], saveplot=False, cmap_name="turbo", box_spec=False
):
...
def unitary_diag(t, H):
    return np.diag(np.exp(-1j * np.diag(H) * t))
